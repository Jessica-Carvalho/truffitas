...
export class LoginService {
  ...
  public async recuperar(email: string) {
    const loading = await this.al.loading();
    this.af.auth.sendPasswordResetEmail(email).then(ok => {
      loading.dismiss();
      this.al.alert("Verifique seu e-mail");
    },
      error => {
        loading.dismiss();
        this.al.toast({ message: 'E-mail inválido' });
      }
    );
  }
